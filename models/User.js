          ...
          new Schema(
            {
			  ...
			  nudgeShown: String, // Nudge was shown/not shown, keep true for all for now, future research will only show the nudge to half of the teens
			  userAction: String, // Action taken by the user, either 'null', 'unhide', 'hide', 'block', 'blockAndReport'
...
userSchema.methods.logUser = function logUser(time, agent, ip) {
  if (this.log.length > 0) {
    const log_time = new Date(this.log[this.log.length - 1].time);
...
userSchema.methods.logPostStats = function logPage(postID) {
  ...
  for (let k = this.feedAction.length - 1; k >= 0; k--) {
...
userSchema.methods.getPosts = function getPosts() {
  ...
  for (let i = 0, len = this.posts.length; i < len; i++) {
    if (this.posts[i].postID >= 0) temp.push(this.posts[i]);
  }

  // sort to ensure that posts[x].postID == x
  temp.sort((a, b) => a.postID - b.postID);
...
userSchema.methods.getPostsAndReplies = function getPostsAndReplies() {
  const temp = [];
  for (let i = 0, len = this.posts.length; i < len; i++) {
    if (this.posts[i].postID >= 0 || this.posts[i].replyID >= 0) { temp.push(this.posts[i]); }
  }

  // sort to ensure that posts[x].postID == x
  temp.sort((a, b) => a.absTime - b.absTime);
...
userSchema.methods.getPostInPeriod = function (min, max) {
  // concat posts & reply
  return this.posts.filter((item) => item.relativeTime >= min && item.relativeTime <= max);
};
