        ...
        .exec((err, script_feed) => {
          ...
          while (script_feed.length || user_posts.length) {
            ...
            if (typeof script_feed[0] === 'undefined') {
              ...
              var feedIndex = _.findIndex(user.feedAction, o => o.post == script_feed[0].id);
                ...
                if (Array.isArray(user.feedAction[feedIndex].comments) && user.feedAction[feedIndex].comments) {
                  ...
                  for (var i = 0; i < user.feedAction[feedIndex].comments.length; i++) {
                    ...
                    if (user.feedAction[feedIndex].comments[i].new_comment) {
                      ...
                      const commentIndex = _.findIndex(script_feed[0].comments, o => o.id == user.feedAction[feedIndex].comments[i].comment);
...
exports.postUpdateFeedAction = (req, res, next) => {
  User.findById(req.user.id, (err, user) => {
    ...
    let feedIndex = _.findIndex(user.feedAction, o => o.post == req.body.postID);
    ...
    // Are we doing anything with a comment?
      ...
      let commentIndex = _.findIndex(user.feedAction[feedIndex].comments, o => o.comment == req.body.commentID);
...
exports.postUpdateProFeedAction = (req, res, next) => {
  User.findById(req.user.id, (err, user) => {
    ...
    // find the object from the right post in feed
    const feedIndex = _.findIndex(user.profile_feed, o => o.profile == req.body.postID);
    ...
    if (feedIndex == -1) {
      ...
      // array of picture_clicks is empty and we have a new (first) picture_clicks event
        ...
        const picture = req.body.picture - user.profile_feed[feedIndex].startTime;
...
exports.postUpdateUserPostFeedAction = (req, res, next) => {
  User.findById(req.user.id, (err, user) => {
    ...
    // Are we doing anything with a comment?
      ...
      const commentIndex = _.findIndex(user.posts[feedIndex].comments, o => o.commentID == req.body.commentID);
      ...
      // LIKE A COMMENT
        ...
        user.posts[feedIndex].comments[commentIndex].liked = !user.posts[feedIndex].comments[commentIndex].liked;
        console.log('^&^&^&^&^&User Post comments LIKE was: ', user.posts[feedIndex].comments[commentIndex].liked);
      }
      ...
      // FLAG A COMMENT
        ...
        user.posts[feedIndex].comments[commentIndex].flagged = !user.posts[feedIndex].comments[commentIndex].flagged;
        console.log('%^%^%^%^%^%User Post comments FLAG was: ', user.posts[feedIndex].comments[commentIndex].flagged);
      }
    }// end of all comment junk
