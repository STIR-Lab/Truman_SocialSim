...
exports.checkBell = (req, res) => {
	if (req.user) {
		...
		Notification.find({
			$or: [{ userPost: user.numPosts }, { actorReply: user.numActorReplies }],
		})
			//Notification.find({ $or: [ { userPost: { $lte: user.numPosts } }, { actorReply: { $lte: user.numActorReplies } } ] })
			.populate("actor")
			.exec(function (err, notification_feed) {
				...
				//When this happens
					...
					for (var i = 0, len = notification_feed.length; i < len; i++) {
						...
						if (notification_feed[i].userPost >= 0) {
							...
							if (
								notification_feed[i].time <= time_diff &&
								notification_feed[i].time > past_diff
							) {
								if (
									notification_feed[i].notificationType == "read" &&
									user.transparency != "no"
								)
									return res.send({ result: true });
								if (notification_feed[i].notificationType != "read")
									return res.send({ result: true });
...
exports.postSignup = (req, res, next) => {
  ...
  ###############################*/
	...
	var result = ["var1", "var2", "var3", "var4"][
		Math.floor(Math.random() * var_num)
	];
...
var sendFinalEmail = function (user) {
	...
	const mailOptions = {
      ...
      Thank you so much for participating in our study!\n
      Your participation has been a huge help in beta testing our app.
      You have one last task to finish the study, and that is to take the final survey here at  ` +
	...
	//we are admin
		...
		User.find()
			...
			.exec(function (err, users) {
              ...
              */
							if (
								users[i].study_days[0] >= 2 &&
								users[i].study_days[1] >= 2 &&
								users[i].study_days[2] >= 2 &&
								users[i].numPosts >= 2
							) {
								users[i].completed = true;
								users[i].save((err) => {
