      ...
      Script.find({ actor: act.id })
        ...
        .exec((err, script_feed) => {
          ...
          for (var i = script_feed.length - 1; i >= 0; i--) {
            var feedIndex = _.findIndex(user.feedAction, o => o.post == script_feed[i].id);
            ...
            if (feedIndex != -1) {
              ...
              if (
                ...
                for (
                  var j = 0;
                  j < user.feedAction[feedIndex].comments.length;
                  j++
                ) {
                  ...
                  if (user.feedAction[feedIndex].comments[j].new_comment) {
                    // comment.new_comment
                    console.log(
                      `adding User Made Comment into feed: ${user.feedAction[feedIndex].comments[j].new_comment_id}`
                    );
                    console.log(
                      JSON.stringify(user.feedAction[feedIndex].comments[j])
                    );

                    const cat = new Object();
                    cat.body =
                      user.feedAction[feedIndex].comments[j].comment_body;
                    cat.new_comment =
                      user.feedAction[feedIndex].comments[j].new_comment;
                    cat.time = user.feedAction[feedIndex].comments[j].time;
                    cat.commentID =
                      user.feedAction[feedIndex].comments[j].new_comment_id;
                    cat.likes = 0;
                    ...
                    const commentIndex = _.findIndex(
                      script_feed[i].comments,
                      o => (
                        o.id == user.feedAction[feedIndex].comments[j].comment
                      )
                    );
                    ...
                    if (commentIndex != -1) {
                      ...
                      if (user.feedAction[feedIndex].comments[j].flagged) {
                        console.log(
                          ...
                          user.feedAction[feedIndex].comments[j].id
                        );
